...
Write-Host "MySQL Setup and Verification Script (Simple)" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
...
try {
    Start-Process -FilePath $mysqlPath -ArgumentList "-u", "root", "-p", "-e", "SELECT 'MySQL connection successful!' as status;" -Wait -NoNewWindow
    Write-Host "✓ MySQL connection test completed" -ForegroundColor Green
} catch {
...
try {
    Start-Process -FilePath $mysqlPath -ArgumentList "-u", "root", "-p", "-e", "CREATE USER IF NOT EXISTS 'root'@'localhost' IDENTIFIED BY 'root'; GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost'; FLUSH PRIVILEGES; CREATE DATABASE IF NOT EXISTS dummy2o;" -Wait -NoNewWindow
...
try {
    Start-Process -FilePath $mysqlPath -ArgumentList "-u", "root", "-p", "-e", "USE dummy2o; SHOW TABLES;" -Wait -NoNewWindow
